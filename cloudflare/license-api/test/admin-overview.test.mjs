...
test("website admin overview batches D1 reads with valid bindings and platform download totals", async () => {
  ...
  assert.equal(payload.releases.released_today, 2);
  assert.deepEqual(payload.downloads.windows, { total: 13, downloads_24h: 5 });
  assert.deepEqual(payload.downloads.apk, { total: 8, downloads_24h: 2 });
  ...
  const db = {
    ...
    async batch(statements) {
      ...
      for (const statement of statements) {
        const placeholderCount = (statement.sql.match(/\?/g) || []).length;
        assert.equal(
          statement.bindings.length,
          placeholderCount,
          `D1 binding mismatch for statement: ${statement.sql}`,
        );
      }
      return [
        result({ total: 2, verified: 2, registered_24h: 1 }),
        result({ total: 1, active: 1, valid: 1, bound_24h: 1 }),
        result({ total: 1, active: 1, draft: 0, suspended: 0, created_today: 1 }),
        result({ total: 9, active: 3, assigned: 1, used: 5, revoked: 0, created_today: 2 }),
        result({ total: 5, active: 4, pending: 0, revoked: 1, expiring_soon: 0, issued_today: 1 }),
        result({ total: 5, latest_released_at: "2026-08-19T00:00:00.000Z", released_today: 2, download_count: 21 }),
        result({
          total_downloads: 21,
          total_24h: 7,
          windows_downloads: 13,
          windows_downloads_24h: 5,
          apk_downloads: 8,
          apk_downloads_24h: 2,
        }),
